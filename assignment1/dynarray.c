...
{
    //put into the nth slot
    void** data2;
    
    if (da -> size >= da -> capacity) {
      da->capacity *= 2;
      data2 = (void**) malloc(sizeof(void*) * da -> capacity);

      //copy over each element from old -> new
    ...
    //this actually does the insert
    da->data[da->size] = val;

    //don't forget to increment the size as well
    da->size++;
    return;
}
