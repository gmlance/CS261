...
void list_insert(struct list* list, void* val)
{
    ...
    //point this new node's next to old head
        node -> next = list -> head;
...
{
        //search the whole linked list for first occurence of val
	    struct node* curr = list->head;
	        struct node* prev = list->head;
	          while (curr != NULL) {
	           //compare val with curr->val, 0 value means equal, means found
	           if (cmp(val, curr->val) == 0) {
	       //if we're removing head, point head to curr next
	                 if (curr == list->head) {
	                       list->head = curr->next;
	                                             }
	           else {
   //curr is the node we want to remove, so update prev's next
                           prev -> next = curr -> next;
	                                                                                              }
	         }
	         free(curr); 
...
void list_reverse(struct list* list)
{
	    ...
	    struct node* prev = NULL;
	        while (curr != NULL) {
		         /*cache old next for future traversal
			  *       */
		         struct node* next = curr->next;

			       /*point curr back to prev (reversing)
				*       */
			       curr->next = prev;

			             /*simple traversals for prev & curr
				      *       */
			             prev = curr;
				           curr = next;
					       }

		    /*at the end, set head to prev
		     *     */
		    list->head = prev;
		        return;
